...
- **すべてのテキスト変換**: スライド内のすべてのテキストを指定されたフォントに変換（デフォルト: Kosugi）
- **英数字変換**: 英数字と一部の記号（%, /, #, @, &, +, -, \*）を指定されたフォントに変換（デフォルト: Montserrat）
  - 全スライド対象
  - 指定ページのみ
- **日本語テキスト変換**: スライド内のすべてのテキストを指定されたフォントに変換（デフォルト: Noto Sans JP）
  - 全スライド対象
  - 指定ページのみ
...
- **フォント設定** (`fonts`)
  - `all.family`: すべてのテキストに適用するフォント（デフォルト: "Kosugi"）
  - `japanese.family`: 日本語テキストに適用するフォント（デフォルト: "Noto Sans JP"）
